...
\begin{Proof} [Division Algorithm]
    ...
    \noindent
    To show ``$r<b$'' we argue by contradiction ``$r\geq b$'' then ``$r=a-bq\geq b$'' subtract $b$ from both sides,
    ``$(r-b=a-bq-b)\geq (b-b$ = 0)'' factoring we see ``$r-b=a-b(q+1)$'' since
    $q+1$ is some integer say $q'$, ``$r-b=a-bq'$'' showing ``$(r-b)<r$'' contradicting our assumption.\\
