...
\begin{Proof} [Division Algorithm]
    ...
    But what if we had some other integer than 0? Let's represent all possibilities by the set:
    \LARGE{$$S=\{b-ax:x\in\mathbb{Z}\}$$}
    \normalsize
    \begin{center}
        Showing how many times $b$ fits into $a$, $x$ times.
    \end{center}

    \noindent
    Let's try some outputs of $S$: ``$a=2$, $b=6$'':\\
    \begin{center}
        \begin{tabular}{c|cc}
            $x$ & $a-bx$                \\
            \hline
        ...
        \end{tabular}
    \end{center}
    Let's not consider negative results such as $x=4$. We'll modify the set $S$ to:
    \LARGE{$$S=\{|b-ax|:x\in\mathbb{Z}\}$$}
    \begin{itemize}
        \item Existence:
