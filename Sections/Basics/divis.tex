...
\begin{Proof} [Division Algorithm]
    ...
    \begin{itemize}
        \item Existence: Take the definition of division ``$a\mid b$'' then ``$b=ax$'', $x$ is
              \textit{some} integer. Note ``$b-ax=0$'', shows we are a whole number and not some decimal. Though what if we weren't wholeNow consider  as this would confirm To account for all \textit{such} integers we construct the set ``$S=\{a-bx:x\in\mathbb{Z}\}$''
