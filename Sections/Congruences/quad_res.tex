...
\noindent
To reduce repetition, we will use $\qres$, to denote Quadratic Residues modulo odd primes $p$.
\begin{theo}[Square Roots $\qres$]
        ...
        \noindent
        I.e., for some $\alpha\in\mathbb{Z}_p^*$, if $\alpha=\beta^2$, then $\alpha$ has two square roots modulo $p$: $\pm\beta$.
\end{theo}
